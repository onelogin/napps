...
/*!
 ...
 @version   1.0
 @copyright Copyright 2015 OneLogin Inc. All rights reserved.
 */
...
/*!
 Checks whether the OneLogin Token Agent is installed on the device.
 Example usage:
 @code
...
/*!
 Checks whether the OneLogin Token Agent is installed on the device and then executes the given block (if any).
 Example usage:
 @code
 [[OneLogin SDK] verifyTokenAgentIsInstalledWithBlock:^(BOOL isInstalled) {
 if (isInstalled) {
 // The OneLogin Token Agent is installed on the device.
 // You could either automatically trigger a NAPPS workflow
 // to SSO the user in, or present the user with a
 // Button that says "Sign on with OneLogin"
 }else{
 // The OneLogin Token Agent is not installed on the device.
 // So you should simply default to your application's regular log-in process.
 }
 }];
 @endcode
